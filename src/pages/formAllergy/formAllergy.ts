  ...
  saveRecordDo(){
    ...
    if (this.card_form.get('recordid').value !==undefined && this.card_form.get('recordid').value !==null) {
      ...
      if (this.card_form.get('severity').dirty){
        this.eventSave.severity = this.card_form.get('severity').value;
      }
...
loadMainRecord() {
  ...
  this.card_form.get('severity').setValue(this.curRec.severity);
  ...
  this.confirmSaveDirect(function(err, result) {
    if (err) {
      ...
      if (result) {
        ...
        self.nav.push(FormAllergyPage, { category: cat, loadFromId: self.curRec.parenteventid });
      } else if (!result) {
