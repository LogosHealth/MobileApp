...
@Component({
  ...
  templateUrl: 'formTherapy.html'
})
export class FormTherapy {
  section: string;
  formName: string = "formTherapy";
  recId: number;
  goalname: string;
  card_form: FormGroup;
  ...
  constructor(public nav: NavController, public alertCtrl: AlertController, public RestService:RestService, public loadingCtrl: LoadingController,
    ...
    if (this.recId !== undefined) {
      ...
      this.therapyname = new FormControl(this.curRec.therapyname, Validators.required);
      this.therapyTerm = this.curRec.therapyname;
      ...
      this.therapyname = new FormControl(null, Validators.required);
  ...
  setFilteredItems() {
    this.items = this.filterItems(this.therapyTerm);
  ...
  fillFormDetails() {
    ...
    this.therapyname.setValue(this.curRec.therapyname);
    this.therapyTerm = this.curRec.therapyname;
    ...
    if ((this.curRec.visitid == undefined || this.curRec.visitid == null) && this.curRec.proceduretiming == 'aftervisit') {
      ...
      if ((this.eventVisit !== undefined && this.eventVisit !== null) || (this.fromVisit !== undefined && this.fromVisit !== null)) {
        console.log('Therapy has visit, but is coming from a visit - eventVisit, ',  this.eventVisit);
        console.log('Therapy has visit, but is coming from a visit - fromVisit, ',  this.fromVisit);
        this.showPhysician = false;
      } else {
        this.hasVisit = true;
        this.showPhysician = false;
        ...
        },
        {
          ...
          handler: () => {
            console.log('Delete clicked');
              ...
              var restURL="https://ap6oiuyew6.execute-api.us-east-1.amazonaws.com/dev/TherapyByProfile";
              var config = {
                invokeUrl: restURL,
                accessKey: this.RestService.AuthData.accessKeyId,
                secretKey: this.RestService.AuthData.secretKey,
              ...
              .then(function(result){
                ...
                self.category.title = "Therapy";
                self.loading.dismiss();
                self.nav.pop();
              }).catch( function(result){
                console.log('Error results from formTherapy.delete: ',result);
                self.loading.dismiss();
              });
          }
        }
  ...
  saveRecordDo(){
    ...
    if (this.card_form.get('recordid').value !==undefined && this.card_form.get('recordid').value !==null) {
      ...
      console.log('fromTherapy save: ', additionalParams);
      ...
      .then(function(result){
        ...
        if (self.visitMode !== undefined && self.visitMode !== null) {
          ...
          self.formSave.namevalue = self.formSave.therapyname;
          self.formSave.type = 'therapy';
          self.dismiss();
        } else {
          self.nav.pop();
        }
      }).catch( function(result){
        console.log('Error results from formTherapy.save: ',result);
        self.loading.dismiss();
      ...
      if (err) {
        ...
        if (result) {
          ...
          if (self.curRec !== undefined && self.curRec.profileid !== undefined && self.curRec.profileid > 0
             && self.curRec.therapyname !== undefined && self.curRec.therapyname !== null) {
            therapyid = self.curRec.recordid;
            therapyname = self.curRec.therapyname;
            medicaleventid = self.curRec.medicaleventid;
            medicalevent = self.curRec.verbatimindication;
          } else if (self.curRec !== undefined && self.curRec.profileid !== undefined && self.curRec.profileid > 0) {
            therapyid = self.curRec.recordid;
            therapyname = self.therapyname.value;
  ...
  searchListTerm(strValue) {
    ...
    this.therapyname.setValue(strValue);
  }
  public today() {
  ...
  presentHelp(myEvent) {
    var title = 'Therapy timing';
    var helptext = "Choose the appropriate therapy timing.  For therapies performed at the diagnosis visit, the proper information will auto-populate<br><br>" +
    "For an upcoming therapy, you can schedule the visit through the schedule a visit button (calendar) after entering the therapy name.";
